#Digit Classification using Convolution Neural Networks on GPUs (with IBM PowerAI) (External resource) 
#Import the MNIST dataset using TensorFlow built-in feature---------------------------------
'''
It's very important to notice that MNIST is a high optimized data-set and it does not contain images. 
You will need to build your own code if you want to see the real digits. 
Another important side note is the effort that the authors invested on this data-set with normalization and centering operations.
'''
...
In this case it will move 2 pixels everytime, thus not overlapping. The input is a matrix of size 28x28x32, and the output would be a matrix of size 14x14x32.
'''
...
So, each matrix [7x7] will be converted to a matrix of [49x1], and then all of the 64 matrix will be connected, which make an array of size [3136x1]. 
We will connect it into another layer of size [1024x1]. So, the weight between these 2 layers will be [3136x1024].
'''
...
for i in range(5000):
    ...
    if i%1000 == 0:
        ...
        print("step", str(i), ", training accuracy", "{:.3f}".format(train_accuracy),", B_time=" , "{:.3f}".format(end - start) )  

#Evaluate the model------------------------------------------------------------------------------
#Print the evaluation to the user with sess.as_default():
